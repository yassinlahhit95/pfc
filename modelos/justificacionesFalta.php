...
function crearJustificacionFalta(int $idAsistencia, int $idEstudiante, string $motivo, ?string $archivo): bool {
    $con  = obtenerConexion();
    $stmt = mysqli_prepare($con,
        "INSERT INTO justificaciones_falta (idAsistencia, idEstudiante, motivo, archivo)
         VALUES (?, ?, ?, ?)");
    mysqli_stmt_bind_param($stmt, "iiss", $idAsistencia, $idEstudiante, $motivo, $archivo);
...
function resolverJustificacionFalta(int $idJustificacion, int $idAsistencia, bool $aprobar, int $idProfesor, string $motivoRechazo = ''): bool {
    $con = obtenerConexion();
    ...
    try {
        ...
        $stmt = mysqli_prepare($con,
            "UPDATE justificaciones_falta
             ...
             WHERE idJustificacion = ? AND estado = 'pendiente'");
        ...
        if (mysqli_stmt_affected_rows($stmt) === 0) throw new \RuntimeException('ya resuelta');

        if ($aprobar) {
            $stmt2 = mysqli_prepare($con, "UPDATE asistencias SET estado = 'justificado' WHERE idAsistencia = ?");
            mysqli_stmt_bind_param($stmt2, "i", $idAsistencia);
