...
if ($method === 'POST') {
    ...
    if ($googleToken) {
        // IP-level rate limiting for Google logins (scope isolated)
        if (!RateLimiter::allow($con, 'api_google_login', 20, 300, 900)) {
            v1Error('Too many login attempts from this IP. Try again in 15 minutes.', 429, 'rate_limited');
        }

        require_once __DIR__ . '/../../include/Security.php';
        $payload = Security::verifyGoogleIdToken($googleToken);
        if (!$payload) {
            v1Error('Invalid or expired Google token.', 401, 'invalid_credentials');
        }
        $email = strtolower(trim($payload['email']));

        // Scan all user tables
        foreach (V1_USER_MAP as $type => [$tabla, $idCol, $emailCol]) {
            ...
            $st = mysqli_prepare($con,
                "SELECT `$idCol` AS uid, $colMustChange
                 FROM `$tabla` WHERE `$emailCol` = ?$filtroEliminado LIMIT 1");
            mysqli_stmt_bind_param($st, 's', $email);
            mysqli_stmt_execute($st);
            $row = mysqli_fetch_assoc(mysqli_stmt_get_result($st));
            if ($row) {
                // Geoblock checks if applicable
                if (in_array($type, ['director', 'secretaria'], true)) {
                    if (FeatureGuard::check('feature_geoblock_admin')) {
                        $ip = Security::clientIp();
                        $country = Security::getCountryFromIP($ip);
                        if ($country !== 'ES') {
                            v1Error('Acceso denegado: Por motivos de seguridad, el panel de administración solo es accesible desde España.', 403, 'geoblock');
                        }
                    }
                }
        ...
        if (!$match) {
            v1Error('This Google account is not registered in the system.', 403, 'account_not_found');
        }
    } else {
