...
$__colorAcento = FeatureGuard::getCenterColor();
$__logoCentro = FeatureGuard::getCenterLogo();
$__colorValid = preg_match('/^#[0-9a-fA-F]{6}$/', $__colorAcento);
?>
<style>
  <?php if ($__colorAcento && $__colorValid): ?>
  :root { --accent: <?= $__colorAcento ?>; }
  <?php endif; ?>
</style>
<div class="brand">
  <?php if ($__logoCentro): ?>
  <img src="/public/uploads/configuracion/<?= htmlspecialchars($__logoCentro) ?>" alt="<?= Security::escapeHtml(FeatureGuard::getCenterName()) ?>" class="brand-logo" style="max-width:40px;height:40px;object-fit:contain;" />
  <?php else: ?>
