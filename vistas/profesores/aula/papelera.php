...
<div class="cabecera">
  <div>
    ...
    <p class="texto-suave" style="margin-top:4px;font-size:0.85rem;"><?= htmlspecialchars($modulo['nombreModulo']) ?> · los elementos se borran automáticamente a los 30 días</p>
  </div>
...
<?php else: ?>
  ...
  <table class="recurso-lista">
    ...
    <tbody>
      ...
      <tr>
        <td><div class="recurso-archivo-nombre"><span class="recurso-carpeta-icono" style="background:<?= htmlspecialchars($c['color']) ?>;width:30px;height:30px;font-size:.8rem;"><i class="fas <?= htmlspecialchars($c['icono']) ?>"></i></span><?= htmlspecialchars($c['nombre']) ?></div></td>
        <td><?= $c['fechaEliminacion'] ? date('d/m/Y H:i', strtotime($c['fechaEliminacion'])) : '—' ?></td>
        <td><div class="recurso-acciones-fila" style="justify-content:flex-end;"><?= botonesPapelera('carpeta', $c['idCarpeta'], $idModulo) ?></div></td>
      </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
  ...
  <table class="recurso-lista">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?= htmlspecialchars($a['nombreCarpeta'] ?? '—') ?></td>
        <td><?= $a['fechaEliminacion'] ? date('d/m/Y H:i', strtotime($a['fechaEliminacion'])) : '—' ?></td>
        <td><div class="recurso-acciones-fila" style="justify-content:flex-end;"><?= botonesPapelera('archivo', $a['idArchivo'], $idModulo) ?></div></td>
      </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
