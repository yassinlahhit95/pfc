...
(function ($) {
    ...
    function crearCampo(clave, def, valor) {
        ...
        switch (def.tipo) {
            ...
            case 'textarea':
                $campo.append($('<textarea rows="3">').attr({ name: clave, maxlength: def.max || 1000 }).val(valor || ''));
                break;
            ...
            case 'imagen':
                $campo.append(crearCampoImagen(clave, valor));
                break;
    ...
    function crearCampoImagen(clave, valor) {
        ...
        if (valor) {
            $preview.append($('<img>').attr('src', '/public/uploads/landing/' + valor));
        } else {
            $preview.append($('<span class="lb-imagen-vacia"><i class="fas fa-image"></i> Sin imagen</span>'));
        }
        var $file = $('<input type="file" accept="image/jpeg,image/png,image/webp" class="lb-imagen-input" style="display:none;">');
    ...
    $(document).on('click', '.lb-imagen-quitar', function () {
        ...
        $wrap.find('.lb-imagen-preview').empty()
            .append($('<span class="lb-imagen-vacia"><i class="fas fa-image"></i> Sin imagen</span>'));
    });
    ...
    $('#lb-form-ajustes').on('submit', function (e) {
        e.preventDefault();
    ...
    $('#lb-publicar').on('click', function () {
        if (!window.confirm('¿Publicar el borrador? Los cambios serán visibles inmediatamente en la web pública.')) return;
        var $boton = $(this).prop('disabled', true);
        post('publicar.php', {})
            .done(function (res) {
                if (!res.ok) { toast(res.msg, 'error'); return; }
    ...
    $('#lb-descartar').on('click', function () {
        if (!window.confirm('¿Descartar los cambios del borrador y volver a la última versión publicada?')) return;
        post('descartar.php', {})
            .done(function (res) {
                if (!res.ok) { toast(res.msg, 'error'); return; }
                toast(res.msg, 'success');
