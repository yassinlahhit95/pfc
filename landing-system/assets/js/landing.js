...
(function () {
  ...
  var secciones = document.querySelectorAll('.lp-sec .lp-tarjeta, .lp-sec-cabecera, .lp-galeria-item, .lp-contacto-grid, .lp-contacto-solo-datos, .lp-cifra, .lp-fpdual-lista li');
  if (secciones.length && 'IntersectionObserver' in window) {
    var obsAnim = new IntersectionObserver(function (entradas) {
      // Agrupar elementos que entran a la vez para hacer cascada
      var entrando = entradas.filter(function(e) { return e.isIntersecting; });
      entrando.forEach(function (entrada, i) {
        // Retraso escalonado (stagger) para tarjetas en la misma fila
        entrada.target.style.transitionDelay = (i * 0.1) + 's';
