...
<table width="100%" cellpadding="0" cellspacing="0" style="border-bottom:3px solid #1e3a6e; padding-bottom:10px; margin-bottom:14px;">
    <tr>
        ...
        <td width="64%" align="center" valign="middle" style="padding:0 10px;">
            ...
            <div style="font-size:14pt; font-weight:700; color:#1e3a6e; margin:3px 0;"><?= htmlspecialchars($cfg['nombreCentro'] ?? 'Centro Educativo') ?></div>
            <?php if (!empty($cfg['codigoCentro'])): ?>
                <div style="font-size:8pt; color:#6b7280;">Código: <?= htmlspecialchars($cfg['codigoCentro']) ?></div>
            <?php endif; ?>
            <?php if (!empty($cfg['direccionCentro'])): ?>
                <div style="font-size:8pt; color:#6b7280;">
                    <?= htmlspecialchars($cfg['direccionCentro']) ?>
                    <?php if (!empty($cfg['ciudadCentro'])): ?> — <?= htmlspecialchars($cfg['ciudadCentro']) ?><?php endif; ?>
                    <?php if (!empty($cfg['telefonoCentro'])): ?> | Tel: <?= htmlspecialchars($cfg['telefonoCentro']) ?><?php endif; ?>
                </div>
            <?php endif; ?>
        </td>
