...
if (isset($_POST['guardarProfesor'])) {
    ...
    if (empty($errores) && checkProfesorExistente($dni, $email)) {
        $errores['dniProfesor'] = "El DNI o correo electrónico especificados ya se encuentran registrados.";
    }
    ...
    if ($idNuevo) {
        ...
        if ($esTutor && $idCicloTutor) {
            actualizarTutorStatus($idNuevo, 1, $idCicloTutor);
        }

        if (isset($_POST['ciclos']) && is_array($_POST['ciclos'])) {
            foreach ($_POST['ciclos'] as $idCic) {
                $idCic = (int)$idCic;
                if ($idCic > 0) asociarCicloProfesor($idCic, $idNuevo);
            }
        }
        if (isset($_POST['modulos']) && is_array($_POST['modulos'])) {
            foreach ($_POST['modulos'] as $idMod) {
                $idMod = (int)$idMod;
                if ($idMod > 0) asociarModuloProfesor($idMod, $idNuevo);
            }
        }
        $_SESSION['exito'] = mensajeExitoConCredenciales("El profesor ha sido registrado correctamente.");
        header("Location: ../../../vistas/admin/profesores/verProfesores.php");
        exit;
    }
    $_SESSION['errores'] = "Ocurrió un error al intentar registrar al profesor.";
    header("Location: ../../../vistas/admin/profesores/agregarProfesores.php");
    exit;
}
