...
class _AddStudentScreenState extends ConsumerState<AddStudentScreen> {
  ...
  void initState() {
    ...
    _codigoPostalController = TextEditingController(text: st?.codigoPostal ?? '');
    _observacionesController = TextEditingController(text: st?.observaciones ?? '');

    _idCiclo = st?.idCiclo;
    _idGrupo = st?.idGrupo;
    _curso = (st?.course != null && st!.course.isNotEmpty) ? st.course : 'Grado Medio';
  ...
  Future<void> _pickDate() async {
    final date = await showDatePicker(
      context: context,
      initialDate: _fechaNacimiento ?? DateTime.now().subtract(const Duration(days: 365 * 15)),
  ...
  Future<void> _save() async {
    ...
    if (_idCiclo == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Debe seleccionar un ciclo')));
      return;
    }
    ...
    try {
      ...
      final newStudent = Student(
        ...
        idCiclo: _idCiclo,
        idGrupo: _idGrupo,
        fechaNacimiento: _fechaNacimiento != null ? DateFormat('yyyy-MM-dd').format(_fechaNacimiento!) : '',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(isEditing ? 'Editar Estudiante' : 'Nuevo Estudiante')),
              ...
              child: Column(
                ...
                children: [
                  ...
                  OutlinedButton.icon(
                    ...
                    label: Text(_fechaNacimiento == null ? 'Fecha de Nacimiento' : DateFormat('dd/MM/yyyy').format(_fechaNacimiento!)),
                  ),
                  const SizedBox(height: Space.lg),
                  TextFormField(
                  ...
                  const Text('Datos Académicos', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  const SizedBox(height: Space.lg),
                  DropdownButtonFormField<int?>(
                    value: _idCiclo,
                    isExpanded: true,
                    decoration: const InputDecoration(labelText: 'Ciclo Formativo', border: OutlineInputBorder()),
                    items: ciclos.map<DropdownMenuItem<int?>>((c) {
                      return DropdownMenuItem(value: c['idCiclo'] as int, child: Text(c['nombreCiclo']));
                    }).toList(),
                    ...
                    children: [
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          items: const [
                            DropdownMenuItem(value: 'Grado Medio', child: Text('Grado Medio')),
                            DropdownMenuItem(value: 'Grado Superior', child: Text('Grado Superior')),
                            DropdownMenuItem(value: 'Grado Básico', child: Text('Grado Básico')),
                          ],
                      ...
                      Expanded(
                        child: DropdownButtonFormField<String?>(
                          value: _anioEstudio,
                          decoration: const InputDecoration(labelText: 'Año (1º o 2º)', border: OutlineInputBorder()),
                          items: const [
                          ...
                          onChanged: (val) => setState(() => _anioEstudio = val),
                        ),
                      ),
                    ],
                  ),
                  ...
                  DropdownButtonFormField<int?>(
                    ...
                    items: [
                      ...
                      ...grupos.map<DropdownMenuItem<int?>>((g) {
                        return DropdownMenuItem(value: g['idGrupo'] as int, child: Text(g['nombreGrupo']));
                      })
                    ],
